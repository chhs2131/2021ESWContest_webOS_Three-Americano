...
1) 서버 구축
2) 파이어베이스 구축 (얼굴데이터, 스케줄, 모드등의 정보 저장)
3) 얼굴 인식 구현 (OPENCV, TensorFlow) - 
4) 음성 인식 구현 (ThinQ API) - 
5) 라즈베리파이 영상 스트리밍 (얼굴인식) - 
6) 웹 프론트앤드 + 날씨 API (UI/UX, 시각화) - 
7) 아두이노 구현부 + MQTT 프로토콜( 스케줄에 맞게 동작 수행, 센싱 및 GPIO 동작 수행)
